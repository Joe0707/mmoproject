    ...
    class GuildService : Singleton<GuildService>, IDisposable
    {
        ...
        private void OnGuildCreate(object sender,GuildCreateResponse response)
        {
            ...
            if(OnGuildCreateResult!=null)
            {
                ...
                MessageBox.Show(string.Format("{0公会创建成功", response.guildInfo.GuildName), "公会");
            }
            else
            {
        ...
        public void SendGuildJointResponse(bool accept,GuildJoinRequest request)
        {
        ...
        private void OnGuildJoinRequest(object sender,GuildJoinRequest request)
        {
            ...
            confirm.OnYes = () =>
            {
                this.SendGuildJointResponse(true, request);
            };
            confirm.OnNo = () =>
            {
                this.SendGuildJointResponse(false, request);
            };
        }
        ...
        {
            Debug.LogFormat("OnGuild:{0}", message.Result);
...
NetMessage();
        ...
        {
            Debug.LogFormat("GuildAdmin {0}{1}");
            MessageBox.Show(string.Format("执行操作{0}结果{1}"));
        }
    }
}
