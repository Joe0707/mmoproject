...
namespace Managers
{
    class ChatManager:Singleton<ChatManager>
    {
        ...
        {
            ChatChannel.Local|ChatChannel.World|ChatChannel.Guild|ChatChannel.Team|ChatChannel.System|
            ChatChannel.Private,
            ChatChannel.Local,
            ChatChannel.World,
            ChatChannel.Team,
        ...
        void StartPrivateChat(int targetId,string targetName)
        {
        ...
        private string FormatMessage(ChatMessage message)
        {
            switch(message.Channel)
            {
                ...
                case ChatChannel.World:
                    return string.Format("<color=cyan>[世界]{0}{1}</color>", FormatFromPlayer(message), message);
                case ChatChannel.System:
                    return string.Format("<color=yellow>[系统]{0}</color>", message.Message);
                case ChatChannel.Private:
                    return string.Format("<color=megenta>[私聊]{0}{1}</color>", FormatFromPlayer(message), message);
                case ChatChannel.Team:
                    return string.Format("<color=green>[队伍]{0}{1}</color>", FormatFromPlayer(message), message);
                case ChatChannel.Guild:
                    return string.Format("<color=blue>[公会]{0}{1}</color>", FormatFromPlayer(message), message);
            }
            return "";
        }
